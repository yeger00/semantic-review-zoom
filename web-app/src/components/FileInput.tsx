...
export default function FileInput({ onLoad }: Props) {
  ...
  function parse(text: string) {
    ...
    if (!review) {
      setError('Could not find a semantic review in this file. Make sure it was generated by /create-semantic-review.')
      return
    }
    setError(null)
    onLoad(review)
  ...
  function handleFile(file: File) {
    if (!file.name.endsWith('.md') && !file.name.endsWith('.txt') && file.type !== 'text/plain') {
      setError('Please upload a .md file generated by /create-semantic-review.')
      return
    }
  ...
  async function handlePaste() {
    ...
    try {
      ...
      if (!text.trim()) {
        setError('Clipboard is empty. Copy the contents of your semantic-review-*.md file first.')
        return
      }
      parse(text)
    } catch {
      ...
      <div style={{ maxWidth: 440, width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <p style={{ color: '#8b949e', fontSize: 14, lineHeight: 1.5 }}>
            Load a <code style={{ background: '#21262d', padding: '1px 5px', borderRadius: 4 }}>semantic-review-*.md</code> file
            generated by <code style={{ background: '#21262d', padding: '1px 5px', borderRadius: 4 }}>/create-semantic-review</code>
          </p>
        </div>
        ...
        >
          ...
          <p style={{ color: '#e6edf3', fontWeight: 600, fontSize: 15, marginBottom: 6 }}>
            {dragging ? 'Drop to load' : 'Drop .md file here'}
          </p>
